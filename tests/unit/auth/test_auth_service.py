...
from app.infrastructure.database.accessor import settings
...
async def test_get_google_redirect_url__success(mock_auth_service: AuthService):
...
async def test_get_yandex_redirect_url__success(mock_auth_service: AuthService):
...
async def test_generate_access_token__success(mock_auth_service: AuthService):
    user_id = str(1)
...
async def test_google_auth__success(mock_auth_service: AuthService):
    ...
    assert isinstance(user, GoogleUserData)
